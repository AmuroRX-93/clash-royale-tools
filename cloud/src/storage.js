...
import {archiveProgress} from './trends.js';
...
export async function archive(sql,tag,player,battles){
 ...
 await archiveProgress(sql,tag,player);
 if(player.name)await sql.query('UPDATE subscriptions SET name=$2 WHERE player_tag=$1',[tag,player.name]);
 return inserts.length;
}
export function decodeHistoryCursor(value,tag){
 if(!value)return null;
 try{if(value.length>512)throw new Error();const c=JSON.parse(value);if(c.tag!==tag||typeof c.time!=='string'||!/^\d{8}T\d{6}(?:\.\d+)?Z?$/.test(c.time)||typeof c.id!=='string'||!c.id.startsWith(tag+'|')||c.id.length>300)throw new Error();return c;}catch{throw new Error('invalid_history_cursor');}
}
export async function history(sql,tag,limit=500,cursorValue=null){
 const cursor=decodeHistoryCursor(cursorValue,tag);
 limit=Math.max(1,Math.min(500,Math.floor(Number(limit)||500)));
 const [allRows,snapshots,total]=await Promise.all([
 sql.query('SELECT id,raw,battle_time,type,mode FROM battles WHERE player_tag=$1'+(cursor?' AND (battle_time,id)<($3,$4)':'')+' ORDER BY battle_time DESC,id DESC LIMIT $2',cursor?[tag,limit+1,cursor.time,cursor.id]:[tag,limit+1]),
 cursor?Promise.resolve([]):sql.query('SELECT ts,trophies,wins,losses,battle_count FROM (SELECT ts,trophies,wins,losses,battle_count FROM snapshots WHERE player_tag=$1 ORDER BY ts DESC LIMIT 2000) s ORDER BY ts ASC',[tag]),
 sql.query('SELECT count(*)::integer AS count FROM battles WHERE player_tag=$1',[tag])]);
 const hasMore=allRows.length>limit,brows=allRows.slice(0,limit);
 ...
 summary.battles=summary.battles.map((b,i)=>({...b,id:brows[i].id}));
 ...
 const last=brows.at(-1),nextCursor=hasMore?JSON.stringify({tag,time:last.battle_time,id:last.id}):null;
 return {...summary,playerTag:tag,totalArchived:brows.length,totalStored:total[0].count,snapshots,ladderTrend,hasMore,nextCursor};
}
